...
export interface TransformViewProps extends ViewProps{
  ...
  magnetic?: boolean,
  tension?: boolean,
  onWillTransform?: Function, //(translateX, translateY, scale)
  onTransforming?: Function, //(translateX, translateY, scale)
  onDidTransform?: Function, //(translateX, translateY, scale)
  onWillInertialMove?: Function, //(translateX, translateY, newX, newY), return ture or false
  onDidInertialMove?: Function, //(translateX, translateY, newX, newY)
  onWillMagnetic?: Function, //(translateX, translateY, scale, newX, newY, newScale), return ture or false
  onDidMagnetic?: Function, //(translateX, translateY, scale)
  onPress?: (e: GestureResponderEvent)=>void, //(event)
  onLongPress?: (e: GestureResponderEvent) => void, //(event)
  style?: ViewStyle;
}
...
export default class TransformView extends Component<TransformViewProps,TransformStatus> {
  static defaultProps = {
    ...ViewBase,
    inertial: true,
    magnetic: true,
  ...
  private lockDirection = 'none';
  private dxSum = 0;
  private dySum = 0;
  private speedX = 0;
  private speedY = 0;
  private touchTime = new Date();
  constructor(props:TransformViewProps) {
    ...
    this.panResponder = PanResponder.create({
      ...
      onPanResponderGrant: (e, gestureState) => this.onPanResponderGrant(e),
      onPanResponderMove: (e, gestureState) => this.onPanResponderMove(e),
      onPanResponderTerminationRequest: (e, gestureState) => true,
      onPanResponderRelease: (e, gestureState) => this.onPanResponderRelease(e),
  ...
  onPanResponderMove(e:GestureResponderEvent) {
    this.handleTouches(e.nativeEvent.touches, (dx:number, dy:number, speedX:number, speedY:number, scaleRate:number) => {
  ...
  handleTouches(touches:NativeTouchEvent[], onHandleCompleted:Function) {
  ...
  handleMagnetic() {
    ...
    let {translateX, translateY, scale} = this.state;
    let newX:any = null, newY:any = null, newScale:any = null;
    if (magnetic) {
      let {x, y, width, height} = this.contentLayout;
  ...
  // AlbumSheet super.onLayout(e) 这里用不了箭头函数
    ...
    return (
      ...
      >
        <Animated.View
          ...
          ref={(ref:any)=>(this.containerView=ref)}
          onLayout={e => {

...
interface WheelItemProps extends ViewProps{
  index: number,
  itemHeight?: number,
  wheelHeight?: number,
...
interface WheelItemState{
  translateY: any,
  scaleX: any,
  scaleY: any,
}
export default class WheelItem extends Component<WheelItemProps,WheelItemState> {
  ...
  constructor(props:WheelItemProps) {
    super(props);
  ...
  componentDidUpdate(prevProps:WheelItemProps) {
  ...
  calcProjection(diameter:number, point:number, width:number) {
  ...
  render() {
    const {style, itemHeight, wheelHeight, index, currentPosition, children, ...others} = this.props;
    ...
    return (
      <Animated.View style={styles} {...others}>
        {children}
      </Animated.View>
    );
  }
